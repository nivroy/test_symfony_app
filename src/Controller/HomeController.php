...
final class HomeController extends AbstractController
{
    ...
    public function start(Request $request): Response
    {
        ...
        if (!preg_match('/^\d{6}$/', $code)) {
            $this->addFlash('error', 'El código debe contener exactamente 6 dígitos.');
            return $this->redirectToRoute('app_home');
        }

        $session = $this->getSession($request);
        if (!$session) {
            throw new \RuntimeException('No hay una sesión activa disponible. ¿Habilitaste "framework.session"?');
        }
    ...
    public function wait(Request $request): Response
    {
        $session = $this->getSession($request);
        if (!$session) {
            $this->addFlash('error', 'La sesión de autenticación no está disponible. Inicia el proceso nuevamente.');
            return $this->redirectToRoute('app_home');
        }

        if (!$session->has('app.auth.name') || !$session->has('app.auth.code')) {
            $this->addFlash('error', 'La autenticación no se pudo continuar. Inténtalo nuevamente.');
            return $this->redirectToRoute('app_home');
        }
        ...
        return $this->render('views/auth_wait.html.twig', [
            'authUrl' => AppVariables::URL_AUTH,
            'name' => $session->get('app.auth.name'),
            'code' => $session->get('app.auth.code'),
        ]);
    }
    ...
    public function confirm(Request $request): Response
    {
        $session = $this->getSession($request);
        if (!$session) {
            $this->addFlash('error', 'La sesión de autenticación no está disponible.');
            return $this->redirectToRoute('app_home');
        }

        if (!$session->has('app.auth.name') || !$session->has('app.auth.code')) {
            $this->addFlash('error', 'La sesión de autenticación ha expirado.');
            return $this->redirectToRoute('app_home');
        }
        ...
        $this->addFlash('success', 'Autenticación completada correctamente. ¡Bienvenido!');
    ...
    public function cancel(Request $request): Response
    {
        ...
        $this->addFlash('warning', 'La autenticación fue cancelada. Puedes intentarlo nuevamente cuando quieras.');
